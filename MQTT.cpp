...
char* ParseforAttribute(String First, String AttributeName, char End, byte* data, unsigned int Length) {
  ; // to find string First then string AttributeName and get chars after that up to Char End in data
  ...
  SentFirstMsg = false;
  SentSecondMsg = false;
  for (int fill = 0; fill <= MaxAttribSize; fill++) {
    FoundAttrib[fill] = 0;   // fill with null
  }

  // for debug
#ifdef _Debug_with_Clock
  if (AttributeName == "cmd=\"") {
    Serial.println();
    Serial.print("starting search for (first) entry  of <");
    Serial.print(First);
    Serial.print("> then < ");
  ...
  for (unsigned int x = 0 ; x <= Length - AttributeName_Length; x++) {
    ...
    for (int w = 0; w <= (First_Length); w++) {
      ...
      //                    }Serial.println("}");}
#endif
      if (First[w] == char(data[x + w])) {
        Firstcount = Firstcount + 1; //loop checks each byte of AttributeName
      }
    ...
    if (FirstFound) {
...
#endif
      allsame = 0;
      for (int y = 0; y <= (AttributeName_Length); y++) {
        Y = y;
        if (AttributeName[y] == char(data[x + y])) {
          allsame = allsame + 1;
#ifdef _attribdebugplus
          //    Serial.print("!");
#endif
      ...
      if (allsame == AttributeName_Length + 1) {
        count = count + 1; // count
      }
      if (count == 1) { // found  nth occurrence of AttributeName
...
#ifdef _Debug_with_Clock
        if (AttributeName == "cmd=\"") {
          Serial.print("found Second text at char<");
          Serial.print(x);
          Serial.print(">  is<");
        }
#endif
        ...
        for (int z = 0 ; z <= MaxAttribSize; z++) {
          ...
          if (data[x + Y + z + 1] == End) {
            FoundAttrib[z] = 0; // end of data with char End eg, 34 == (")
            for (int fill = z; fill <= MaxAttribSize; fill++) {
              FoundAttrib[fill] = 0;   // not needed with filled string with '0' ?
            }
            // for debug
#ifdef _Debug_with_Clock
            if (AttributeName == "cmd=\"") {
  ...
  for (unsigned int x = 0 ; x <= Length - id_Length - 20; x++) {
    ...
    if (count == Nth) {
      ...
      for (int z = 0 ; z <= MaxAttribSize; z++) {
        //FoundAttrib[z]=char(data[x+Y+z+1]);
        if (data[x + Y + z + 1] == End) {
          FoundAttrib[z] = 0; // end of data with ascii dec 34 == (")
          for (int fill = z; fill <= MaxAttribSize; fill++) {
            FoundAttrib[fill] = 0;   // not needed with filled string with '0' ?
          }
          // for debug
...
void ParseSwitchList(byte Switch, byte* payload, unsigned int Length) {
  ...
  if (SwitchNumbers >= MAXLOCOS - 1) {
    SwitchNumbers = MAXLOCOS - 1;
  }
...
void MQTTFetch (char* topic, byte* payload, unsigned int Length) { //replaces original rocfetch
  ...
  if ((strncmp("rocnet/ps", topic, 9) == 0)) { //==0 if n bytes are the same this is a "Programming stationary" message
    ...
    if (IntFromPacket_at_Addr(payload, Sender_Addr) == SW_bus[switchindex]) { // Serial.print("this is the correct sender/node");
      if (ROC_Data[1] == SW_addr[switchindex]) { // correct addr/pin
        ...
        for (byte x = 1; x <= 8; x++) {
          SW_all[x] = ROC_Data[x]; //store the lot so we can send it back with modifified on and off pos from menu
        }
        Message_Decoded = true;
      }
    }
    ...
    Message_Decoded = true;
    if   ((!Message_Decoded)) {
      Serial.print("Unknown rocnet/ps MSG Length<");
      Serial.print(Length);
      Serial.print("> is ");
      Serial.print(Show_ROC_MSG(Length));
    }
  ...
  if ((strncmp("rocrail/service/info", topic, 20) == 0)) {
    if (strlen(Attrib('\"', "<lc ", payload, Length)) >= 1) {
      ThisID = Attrib('\"', "id=\"", payload, Length);
    ...
    if ((!Message_Decoded) && (strlen(Attrib('\"', "<lc id=\"", payload, Length)) >= 1) && (strlen(Attrib('\"', "prev_id=\"", payload, Length)) >= 1)) { // {does it contain "<lc id="  and "prev_id=" as well is should only capture lcprops messages?
      ...
      // Serial.println("");
      same = false;
      for (int check = 0; check <= MAXLOCOS ; check++) {
        if ((ThisID == LOCO_id[check])) {
          same = true;
        }
      }
      ...
      if (!same) { // NEW ...added check for show="true";  and active="true"
        ...
        if (_Show == "true") { //was previously if ((_Show=="true")&&(_Active=="true")){
          ...
          // LocoNumbers++;
          if (LocoNumbers >= MAXLOCOS - 1) {
            LocoNumbers = MAXLOCOS - 1; // overwrite the last number if too many locos
          }
  ...
  if (strncmp("<clock ", PayloadAscii, 7) == 0) {
    ...
    if (CmdMsg == "go") {
      Clock_Freeze = false;  // save the time the the clock was frozen....
  ...
  if (!Message_Decoded) {
    if ((strncmp("rocnet", topic, 6) == 0)) {
      Serial.println("");
      Serial.print("Unknown rocnet MSG is topic<");
      Serial.print(topic);
      Serial.print(">  <");
      Serial.print(Show_ROC_MSG(Length));
    }
...
void DebugMsgSend (String topic, String Debug_Payload) { // use with mosquitto_sub -h 127.0.0.1 -i "CMD_Prompt" -t debug -q 0
  ...
  if ((hrs == 0) && (mins == 0)) { // not Synchronised yet..
    ...
    cx = sprintf(DebugMsgLocal, "<%02d:%02d:%02ds> %s", hrs, mins, secs, DebugMsgTemp);
  }
  //
...
void MQTT_ReConnect() {
  ...
  while (!client.connected()) {
    if ( connects == 0) {
      PrintTime("---Attempting MQTT Connection" );
    }
    else {
