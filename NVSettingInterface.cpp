...
void CheckForSerialInput() {
  ...
  if (wifiSSID == "Router Name") {
    UpdateInProgress = true; Serial.println(" Forcing request for new entries as Default Router name has not been set in Secrets.h");
    ...
    Serial.println("");
    Serial.println(F("--- To enter new wifi details / Press STOP or type 'xxx' BEFORE wifi connects"));
  ...
  while ((millis() <= Timestarted + 4000) || UpdateInProgress) {
    ///   toggle  signal lamp
    if ((millis() >= FlashTime) && (!UpdateInProgress)) {
      LAMP = !LAMP;
      FlashTime = millis() + 50;
      digitalWrite (NodeMCUPinD[0] , LAMP) ;
      Serial.print("~");
    }
    ...
    recvWithEndMarker();
    /*
    if ( (StopPressed() && (SerioLevel == 0) ) || (newData == true) )
    {
      if (newData == true) {
        TestData = receivedChars;
        _ProcessedSerialInput = true;
        ndx = 0;
        newData = false;
      }
      ...
      switch (SerioLevel) {
        case 0:
          if ((TestData == "xxx\0") || (TestData == "XXX\0") || StopPressed()) {
          ...
          break;
        case 1: if (MSG_content_length() > 1) {
            wifiSSID = receivedChars;
          } newData = false; SerioLevel = 2;
          MSGText1 = "SSID     <";
          MSGText1 += wifiSSID;
          MSGText1 += ">";
        ...
        case 2:
          if (MSG_content_length() > 1) {
            wifiPassword = receivedChars;
          } newData = false; SerioLevel = 3;
          MSGText2 = "Password <";
        ...
        case 3:
          if (MSG_content_length() > 1) {
            BrokerAddr = TestData.toInt();
          } newData = false; SerioLevel = 4;
        ...
        case 4:
          if (MSG_content_length() > 1) {
            NameOfThisThrottle = receivedChars;
        ...
        case 8:
          ...
          if ((cal_input >= 200) && (cal_input <= 600)) { // valid input
            ...
            Serial.println("- unchanged calibration factor -");
          }
          analog_sum = ReadADC(100); //
...
void recvWithEndMarker() {
  ...
  while (Serial.available() > 0 ) {
    ...
    if ((rc != 10) && (rc != 13) ) {
      ...
      ndx++;
      if (ndx >= numChars) {
        ndx = numChars - 1; //truncate if too bit to avoid filling up
      }
    }
    else {
    ...
    delay(1);
  }   // added delay to allow other processes whilst reading serial.
